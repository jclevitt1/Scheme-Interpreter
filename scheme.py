...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        try:
            ...
            return evaluated_operator.eval_call(rest, env)
        except:
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        def recursePairs(formals, vals):
            if formals == nil:
                return
            child.define(formals.first, vals.first)
            return recursePairs(formals.second, vals.second)
        recursePairs(formals, vals)
        # END PROBLEM 11
        return child
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        application = scheme_apply(self, operands, env)
        return scheme_eval(complete_eval(application), env, True)
        # END Problem 21
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        fn = target.first
        args = target.second
        env.define(fn, LambdaProcedure(args, expressions.second, env))
        return fn
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        parameters = Pair(first.first, parameters)
        values = Pair(scheme_eval(first.second.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    # BEGIN PROBLEM 10
        fn = target.first
        args = target.second
        env.define(fn, MacroProcedure(args, expressions.second, env))
        return fn
    # END PROBLEM 10
    else:
        bad_target = target.first if isinstance(target, Pair) else target
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            evaluated_operator = scheme_eval(first, env)
            check_procedure(evaluated_operator)
            result = evaluated_operator.eval_call(rest, env)
            # END PROBLEM 20
    return result
